...
select storekey ,profit, case
                    when profit > 10000 then 'yes yes yes '
                    when profit >1000 and profit < 9999 then 'ok ok ok'
					when profit < 1000 then 'nooooooo' 
					end as yo
from  (
select StoreKey,
count(SalesQuantity*UnitPrice-(DiscountAmount*DiscountQuantity)-(SalesQuantity*UnitCost)) 
as profit 
from [ContosoRetailDW].[dbo].[FactSales] 
group by StoreKey)as profit_2;
